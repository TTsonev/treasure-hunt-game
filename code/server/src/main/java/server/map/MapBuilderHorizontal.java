...
public class MapBuilderHorizontal extends AMapBuilder {
	private static final int X_SHIFT_HORIZONTAL = 10;
	@Override
	...
	protected Map<Position, ServerMapNode> shiftHalfMap(Map<Position, ServerMapNode> halfmap, EPlayerNumber playerNum) {
		...
		for (ServerMapNode mapNode : halfmap.values()) {
			Position shiftedPosition = new Position(mapNode.getX()+X_SHIFT_HORIZONTAL, mapNode.getY());
